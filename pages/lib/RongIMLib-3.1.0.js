...
(function (OperateType) {
    /** 私有云专用，解密协商指令 */
    OperateType[OperateType["SYMMETRIC"] = 0] = "SYMMETRIC";
    /** 连接请求 */
    OperateType[OperateType["CONNECT"] = 1] = "CONNECT";
    /** 连接应答 */
    OperateType[OperateType["CONNACK"] = 2] = "CONNACK";
    /** 上行发送消息 */
    OperateType[OperateType["PUBLISH"] = 3] = "PUBLISH";
    /** 上行发送消息的应答 */
    OperateType[OperateType["PUBACK"] = 4] = "PUBACK";
    /** 上行拉消息 */
    OperateType[OperateType["QUERY"] = 5] = "QUERY";
    /** 上行拉消息的应答 */
    OperateType[OperateType["QUERYACK"] = 6] = "QUERYACK";
    OperateType[OperateType["QUERYCON"] = 7] = "QUERYCON";
    OperateType[OperateType["SUBSCRIBE"] = 8] = "SUBSCRIBE";
    OperateType[OperateType["SUBACK"] = 9] = "SUBACK";
    OperateType[OperateType["UNSUBSCRIBE"] = 10] = "UNSUBSCRIBE";
    OperateType[OperateType["UNSUBACK"] = 11] = "UNSUBACK";
    OperateType[OperateType["PINGREQ"] = 12] = "PINGREQ";
    OperateType[OperateType["PINGRESP"] = 13] = "PINGRESP";
    /** 连接挂断 */
    OperateType[OperateType["DISCONNECT"] = 14] = "DISCONNECT";
    OperateType[OperateType["RESERVER2"] = 15] = "RESERVER2";
...
class Header {
    constructor(type, retain = false, qos = QOS.AT_LEAST_ONCE, dup = false) {
        ...
        if (type && isPlusType && arguments.length === 1) {
            this.retain = (type & 1) > 0;
            this.qos = (type & 6) >> 1; // (_type & 0b110) >> 1
            this.dup = (type & 8) > 0; // (_type & 0b1000) > 0
    ...
    encode() {
        ...
        byte |= this.retain ? 1 : 0;
        byte |= this.qos << 1;
        byte |= this.dup ? 8 : 0; // 8 -> 1000
        return byte;
    }
}
/**
...
class RongStreamReader {
    ...
    readInt() {
        ...
        let end = '';
        for (let i = 0; i < 4; i++) {
            let t = self.pool[self.position++].toString(16);
            if (t.length === 1) {
                t = '0' + t;
            }
            end += t.toString();
    ...
    readByte() {
        ...
        let val = this.pool[this.position++];
        if (val > 255) {
            val &= 255;
        }
        return val;
    ...
    readUTF() {
        ...
        const pool = this.pool.subarray(this.position, this.position += big);
...
class RongStreamWriter {
    ...
    write(byte) {
        ...
        if (Object.prototype.toString.call(byte).indexOf('Array') !== -1) {
            this.pool = this.pool.concat(byte);
        }
        else if (byte >= 0) {
            if (byte > 255) {
                byte &= 255;
...
const defaultStdout = (level, ...args) => {
    level >= crtLevel && methods[level]('RCLog:', ...args);
};
let crtStdout = defaultStdout;
let crtLevel =  LogLevel.DEBUG ;
const set = (level, stdout) => {
    crtLevel = level;
    crtStdout = stdout ? (logLevel, ...args) => {
        logLevel >= crtLevel && stdout(...args);
    } : defaultStdout;
};
const log = (level, ...args) => {
    crtStdout(level, ...args);
};
const debug = log.bind(null, LogLevel.DEBUG);
const info = log.bind(null, LogLevel.INFO);
const warn = log.bind(null, LogLevel.WARN);
const error = log.bind(null, LogLevel.ERROR);
var logger = {
    set,
    log,
    debug,
...
class DataCodec {
    constructor(connectType) {
        // this.codec = connectType === 'websocket' ? SocketCodec : CometCodec
        this.codec = connectType === 'websocket' ? Codec : Codec;
    ...
    */
    decodeByPBName(data, pbName, option) {
        ...
        const formatEventMap = {
            [PBName.DownStreamMessages]: self.formatSyncMessages,
            [PBName.DownStreamMessage]: self.formatReceivedMessage,
            [PBName.UpStreamMessage]: self.formatSentMessage,
            [PBName.HistoryMsgOuput]: self.formatHistoryMessages,
            [PBName.RelationsOutput]: self.formatConversationList,
            [PBName.QueryChatRoomInfoOutput]: self.formatChatRoomInfos,
            [PBName.RtcUserListOutput]: self.formatRTCUserList,
            [PBName.RtcQryOutput]: self.formatRTCData,
            [PBName.ChrmKVOutput]: self.formatChatRoomKVList,
            [PBName.PullUserSettingOutput]: self.formatUserSetting,
            [PBName.SessionStates]: self.formatConversationStatus
        };
        let decodedData = data;
        const formatEvent = formatEventMap[pbName];
        try {
            const hasData = data.length > 0; // 判断是否有数据, 防止无数据 pb 解析报错
            decodedData = hasData && self.codec[pbName].decode(data); // pb 解析
    ...
    formatBytes(content) {
        ...
        let formatRes = this.readBytes(content);
        try {
     ...
     * 格式化多端同步消息
    */
    formatSyncMessages(data, option) {
        option = option || {};
        const self = this;
        ...
        data.list = map(list, (msgData) => {
            const message = self.formatReceivedMessage(msgData, option);
            return message;
        });
        return data;
    }
     ...
     * 格式化接收消息
    */
    formatReceivedMessage(data, option) {
        ...
        const utfContent = self.formatBytes(content);
        ...
        // logger.warn('formatReceivedMessage', utfContent)
        return {
    ...
    */
     ...
     * 格式化历史消息
    */
    formatHistoryMessages(data, option) {
        ...
        forEach(msgList, (msgData) => {
            const msg = this.formatReceivedMessage(msgData, option);
     ...
     * 格式化会话列表
    */
    formatConversationList(serverData, option) {
        const self = this;
        ...
        conversationList = map(conversationList, (serverConversation) => {
            ...
            const latestMessage = self.formatReceivedMessage(msg, option);
    ...
    */
    encodeServerConfParams() {
        const modules = this.codec.getModule(PBName.SessionsAttQryInput);
     ...
     * 上行消息基础配置
    */
    getUpMsgModule(conversation, option) {
        ...
        const modules = this.codec.getModule(PBName.UpStreamMessage);
    ...
    */
    encodeUpMsg(conversation, option) {
        const modules = this.getUpMsgModule(conversation, option);
        return modules.getArrayData();
    }
    /**
     * 序列化拉取多端消息
    */
    encodeSyncMsg(syncMsgArgs) {
        ...
        const modules = this.codec.getModule(PBName.SyncRequestMsg);
    ...
    */
    encodeChrmSyncMsg(time, count) {
        time = time || 0;
        count = count || 0;
        const modules = this.codec.getModule(PBName.ChrmPullMsg);
    ...
    */
    encodeGetHistoryMsg(targetId, option) {
        ...
        const modules = this.codec.getModule(PBName.HistoryMsgInput);
    ...
    */
    encodeGetConversationList(option) {
        ...
        const modules = this.codec.getModule(PBName.RelationQryInput);
    ...
    */
    encodeOldConversationList(option) {
        option = option || {};
        let { count, type } = option;
        count = count || 0; // 删除会话列表 count 传 0 , setCount 形参 count 为必填参数
        const modules = this.codec.getModule(PBName.RelationsInput);
    ...
    */
    encodeRemoveConversationList(conversationList) {
        const modules = this.codec.getModule(PBName.DeleteSessionsInput);
        const sessions = [];
        forEach(conversationList, (conversation) => {
            const { type, targetId } = conversation;
            const session = this.codec.getModule(PBName.SessionInfo);
    ...
    */
    encodeDeleteMessages(conversationType, targetId, list) {
        const modules = this.codec.getModule(PBName.DeleteMsgInput);
        const encodeMsgs = [];
    ...
    */
    encodeClearMessages(targetId, timestamp) {
        const modules = this.codec.getModule(PBName.CleanHisMsgInput);
    ...
    */
    encodeClearUnreadCount(conversation, option) {
        ...
        const modules = this.codec.getModule(PBName.SessionMsgReadInput);
    ...
    */
    encodeJoinOrQuitChatRoom() {
        const modules = this.codec.getModule(PBName.ChrmInput);
        modules.setNothing(1);
        return modules.getArrayData();
    }
    /**
    ...
    */
    encodeGetChatRoomInfo(count, order) {
        const modules = this.codec.getModule(PBName.QueryChatRoomInfoInput);
        modules.setCount(count);
        modules.setOrder(order);
        return modules.getArrayData();
    }
    ...
    */
    encodeGetFileToken(fileType, fileName) {
        const modules = this.codec.getModule(PBName.GetQNupTokenInput);
    ...
    */
    encodeGetFileUrl(fileType, fileName, originName) {
        const modules = this.codec.getModule(PBName.GetQNdownloadUrlInput);
        modules.setType(fileType);
        modules.setKey(fileName);
        if (originName) {
    ...
    */
    encodeModifyChatRoomKV(chrmId, entry, currentUserId) {
        const modules = this.codec.getModule(PBName.SetChrmKV);
        ...
        if (isSendNotification) { // 如果需要发送通知, 设置通知消息
            ...
            const msgModule = this.getUpMsgModule(conversation, {
    ...
    */
    encodePullChatRoomKV(time) {
        const modules = this.codec.getModule(PBName.QueryChrmKV);
    ...
    */
    encodePullUserSetting(version) {
        const modules = this.codec.getModule(PBName.PullUserSettingInput);
    ...
    */
    encodeGetConversationStatus(time) {
        const modules = this.codec.getModule(PBName.SessionReq);
    ...
    */
    encodeSetConversationStatus(statusList) {
        const isComet = this.connectType === 'comet';
        const modules = this.codec.getModule(PBName.SessionStateModifyReq);
        const currentTime = DelayTimer.getTime();
        const stateModuleList = [];
        forEach(statusList, (status) => {
            const stateModules = this.codec.getModule(PBName.SessionState);
            ...
            forEach(TypeToVal, (val, type) => {
                if (!isUndefined(val)) {
                    const stateItemModules = this.codec.getModule(PBName.SessionStateItem);
                    ...
                    const stateItemModulesData = isComet ? stateItemModules.data : stateItemModules;
                    stateItemModuleList.push(stateItemModulesData);
                }
            });
            stateModules.setStateItem(stateItemModuleList);
            const stateModulesData = isComet ? stateModules.data : stateModules;
    ...
    encodeJoinRTCRoom(mode, broadcastType) {
        const modules = this.codec.getModule(PBName.RtcInput);
        mode = mode || 0;
    ...
    */
    encodeSetRTCData(key, value, isInner, apiType, message) {
        const modules = this.codec.getModule(PBName.RtcSetDataInput);
    ...
    */
    encodeUserSetRTCData(message, valueInfo, objectName) {
        const modules = this.codec.getModule(PBName.RtcUserSetDataInput);
        // 全量 URI 新增
        // 全量发布中
        ...
        // content
        let val = this.codec.getModule(PBName.RtcValueInfo);
    ...
    */
    encodeGetRTCData(keys, isInner, apiType) {
        const modules = this.codec.getModule(PBName.RtcDataInput);
        modules.setInterior(isInner);
        modules.setTarget(apiType);
        modules.setKey(keys);
        return modules.getArrayData();
    ...
    */
    encodeSetRTCOutData(data, type, message) {
        const modules = this.codec.getModule(PBName.RtcSetOutDataInput);
    ...
    */
    ecnodeGetRTCOutData(userIds) {
        const modules = this.codec.getModule(PBName.RtcQryUserOutDataInput);
        modules.setUserId(userIds);
        return modules.getArrayData();
    }
    encodeSetRTCState(report) {
        const modules = this.codec.getModule(PBName.MCFollowInput);
    ...
    */
    encodeGetRTCRoomInfo() {
        const modules = this.codec.getModule(PBName.RtcQueryListInput);
        modules.setOrder(2);
        return modules.getArrayData();
    }
    /**
    ...
    */
    encodeSetRTCUserInfo(key, value) {
        const modules = this.codec.getModule(PBName.RtcValueInfo);
        modules.setKey(key);
        modules.setValue(value);
        return modules.getArrayData();
    }
    ...
    */
    encodeRemoveRTCUserInfo(keys) {
        const modules = this.codec.getModule(PBName.RtcKeyDeleteInput);
        modules.setKey(keys);
        return modules.getArrayData();
    }
}
...
const getReaderByHeader = (header) => {
    ...
    let msg;
    switch (type) {
        case OperateType.CONNACK:
            msg = new ConnAckReader(header);
            break;
        case OperateType.PUBLISH:
            msg = new PublishReader(header);
            msg.syncMsg = header.syncMsg;
            break;
        case OperateType.PUBACK:
            msg = new PubAckReader(header);
            break;
        case OperateType.QUERYACK:
            msg = new QueryAckReader(header);
            break;
        case OperateType.SUBACK:
        case OperateType.UNSUBACK:
        case OperateType.PINGRESP:
...
const formatWSUrl = (protocol, host, appkey, token, runtime) => {
    return `${protocol}://${host}/websocket?appId=${appkey}&token=${encodeURIComponent(token)}&sdkVer=${"3.1.0"}&apiVer=${ 'normal'}${runtime.connectPlatform ? '&platform=' + runtime.connectPlatform : ''}`;
};
...
class WebSocketChannel extends ADataChannel {
    ...
    constructor(runtime, watcher) {
        ...
        this.isStatusMessage = (topic) => {
            return [Topic$1.ppMsgS, Topic$1.pgMsgS, Topic$1.chatMsgS].map(item => Topic$1[item]).indexOf(topic) >= 0;
        };
        // TODO: 需确认 v3 中 messageId 超出 65535 抛异常的原因
        this.idCount = 0;
        this.generateMessageId = () => {
            return ++this.idCount;
        };
    }
    connect(appkey, token, hosts, protocol) {
        return __awaiter(this, void 0, void 0, function* () {
            // 通知连接中
            this.watcher.status(ConnectionStatus$1.CONNECTING);
            // 检索有效地址
            const validHosts = yield getValidHosts(hosts, protocol, this.runtime);
            ...
            for (let i = 0, len = validHosts.length; i < len; i += 1) {
                const url = formatWSUrl(wsProtocol, validHosts[i], appkey, token, this.runtime);
                // 创建 socket，若超时一定时间未收到 ConnAck 确认，则视为连接超时
                const socket = this.runtime.createWebSocket(url);
                // 服务连接非主动断开，尝试重连
                const disconnected = (code) => {
                    if (this.socket === socket) {
                        this.socket = null;
                        this.watcher.status(code);
                    }
                };
                // 等待连接结果
                const code = yield new Promise((resolve) => {
                    socket.onMessage((data) => {
                        ...
                        // Ping 响应
                        if (signal instanceof PingRespReader && this.pingResolve) {
                            this.pingResolve(ErrorCode$1.SUCCESS);
                        ...
                        if (signal instanceof DisconnectReader) {
                            ...
                            this.watcher.status(connStatus);
                            return;
                        }
                        // 非连接信令处理
                        this.onReceiveSignal(signal);
                    });
                ...
                if (code === ErrorCode$1.SUCCESS) {
                    this.socket = socket;
                    // 通知上层连接成功
                    this.watcher.status(ConnectionStatus$1.CONNECTED);
                    // 启动定时心跳
                    this.checkAlive();
                    return code;
                }
    ...
    checkAlive() {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // 响应超时，尝试关闭连接
            if (code !== ErrorCode$1.SUCCESS && ++this.failedCount >= this.ALLOW_FAILED_TIMES) {
                (_a = this.socket) === null || _a === void 0 ? void 0 : _a.close();
                return;
            }
            this.failedCount = 0;
            // 重新定时任务
            setTimeout(() => this.checkAlive(), IM_PING_INTERVAL_TIME);
        });
    }
    onReceiveSignal(signal) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (messageId > 0) {
                const resolve = this.messageIds[key];
                resolve && resolve(signal);
                // 多端同步消息的 pubAck
                this.syncMessageIds[key] && this.syncMessageIds[key](signal);
            }
            // PublishReader 处理
            if (signal instanceof PublishReader) {
                const { syncMsg, topic } = signal;
                // 非同步消息或者是状态消息（ppMsgS，pgMsgS，chatMsgS），则直接抛出到上层
                if (!syncMsg || this.isStatusMessage(topic)) {
                    this.watcher.signal(signal);
                    return;
                }
                // 多端同步消息息需等待 CMP 发送的 PubAck（Comet 不发）
                const ack = yield new Promise(resolve => {
                    this.syncMessageIds[key] = resolve;
                });
                delete this.syncMessageIds[key];
        ...
        return __awaiter(this, void 0, void 0, function* () {
            if (this.socket) {
                ...
                const respSignal = yield new Promise((resolve) => {
                    const key = formatResolveKey(messageId, identifier);
                    this.messageIds[key] = resolve;
                    setTimeout(() => {
                        delete this.messageIds[key];
    ...
    close() {
        if (this.socket) {
            this.socket.close();
            this.socket = null;
            this.watcher.status(ConnectionStatus$1.DISCONNECTED);
        }
    }
}
    ...
    httpReq(options) {
        ...
        return new Promise((resolve) => {
            wx.request({
                ...
                fail: () => {
                    resolve({ status: ErrorCode$1.RC_HTTP_REQ_TIMEOUT });
                }
            });
        });
    },
...
const { localStorage: storage$1 } = runtime$1;
const getKey = (appkey, token) => {
    return ['navi', appkey, token].join('_');
};
const getNaviInfoFromCache = (appkey, token) => {
    const key = getKey(appkey, token);
    const jsonStr = storage$1.getItem(key);
    if (!jsonStr) {
        return null;
    }
    let data;
...
const setNaviInfo2Cache = (appkey, token, naviInfo) => {
    const key = getKey(appkey, token);
    const data = {
        naviInfo,
        timestamp: Date.now()
    };
    storage$1.setItem(key, JSON.stringify(data));
};
const clearCache = (appkey, token) => {
    storage$1.removeItem(getKey(appkey, token));
};
const formatNaviUrl = (url, token, appkey, jsonpFunc) => {
    const path = runtime$1.isSupportSocket() ? 'navi' : 'cometnavi';
    const tmpUrl = `${url}/${path}.js?appId=${appkey}&token=${encodeURIComponent(token)}&callBack=${jsonpFunc}&v=${"3.1.0"}&r=${Date.now()}`;
    return tmpUrl;
};
const reqNavi = (uris, appkey, token) => __awaiter(void 0, void 0, void 0, function* () {
    const jsonpFunc = 'getServerEndpoint';
    for (let i = 0, len = uris.length; i < len; i += 1) {
        const url = formatNaviUrl(uris[i], token, appkey, jsonpFunc);
        logger.debug(`req navi => ${url}`);
        const res = yield runtime$1.httpReq({
            url,
...
class Navi {
     ...
     */
    navigators, customCMP = []) {
        this.appkey = appkey;
        this.navigators = navigators;
        this.customCMP = customCMP;
    }
    /**
     * 获取导航数据
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!runtime$1.useNavi) {
                const naviInfo = {
                    ...
                    voipServer: '',
                    offlinelogserver: '',
                    backupServer: ((_a = this.customCMP) === null || _a === void 0 ? void 0 : _a.length) ? this.customCMP.join(',') : 'wsproxy.cn.ronghub.com'
                };
                setNaviInfo2Cache(this.appkey, token, naviInfo);
                return naviInfo;
            }
            ...
            // 判断是否有有效缓存数据
            let naviInfo = getNaviInfoFromCache(this.appkey, token);
            if (naviInfo) {
                return naviInfo;
            }
            const uris = this.navigators.slice();
            dynamicUris.length && dynamicUris.forEach(uri => {
                uris.indexOf(uri) < 0 && uris.push(uri);
            });
            // 串行请求，直到获取到导航数据或所有请求结束
            // TODO: 考虑是否可改为并行请求，串行请求时间过长
            naviInfo = yield reqNavi(uris, this.appkey, token);
            if (naviInfo) {
                setNaviInfo2Cache(this.appkey, token, naviInfo);
                return naviInfo;
            }
    ...
    getInfoFromCache(token) {
        return getNaviInfoFromCache(this.appkey, token);
    }
    /**
     * 清空导航数据：内存数据、缓存数据
     */
    clear(token) {
        clearCache(this.appkey, token);
    }
}
...
class PluginContext {
     ...
     */
    getAppkey() {
        return this.context.appkey;
    }
    /**
     * 获取当前运行中的 IM 引擎版本号
     */
    getEngineVersion() {
        return this.context.coreVersion;
    }
    /**
     * 获取当前已连接用户的 userId
     ...
     */
    getNaviInfo() {
        return this.context.getInfoFromCache();
    }
    /**
     * 发送消息
     */
    ...
    /**
     * 获取其他依赖插件
     * @param pluginTag 插件标识
     */
    getPlugin(pluginTag) {
        return this.context.pluginApi[pluginTag];
    }
    /**
     * 消息注册
    ...
    registerMessageType(objectName, isPersited, isCounted, searchProps = []) {
        this.context.registerMessageType(objectName, isPersited, isCounted, searchProps);
    }
}
...
class RTCPluginContext extends PluginContext {
    ...
    joinRTCRoom(roomId, mode, broadcastType) {
        return this.context.joinRTCRoom(roomId, mode, broadcastType);
    }
    quitRTCRoom(roomId) {
        return this.context.quitRTCRoom(roomId);
    }
    rtcPing(roomId, mode, broadcastType) {
        return this.context.rtcPing(roomId, mode, broadcastType);
    }
    getRTCRoomInfo(roomId) {
        return this.context.getRTCRoomInfo(roomId);
    }
    getRTCUserInfoList(roomId) {
        return this.context.getRTCUserInfoList(roomId);
    }
    getRTCUserInfo(roomId) {
        return this.context.getRTCUserInfo(roomId);
    }
    setRTCUserInfo(roomId, key, value) {
        return this.context.setRTCUserInfo(roomId, key, value);
    }
    removeRTCUserInfo(roomId, keys) {
        return this.context.removeRTCUserInfo(roomId, keys);
    }
    setRTCData(roomId, key, value, isInner, apiType, message) {
        return this.context.setRTCData(roomId, key, value, isInner, apiType, message);
    }
    setRTCTotalRes(roomId, message, valueInfo, objectName) {
        return this.context.setRTCTotalRes(roomId, message, valueInfo, objectName);
    }
    getRTCData(roomId, keys, isInner, apiType) {
        return this.context.getRTCData(roomId, keys, isInner, apiType);
    }
    removeRTCData(roomId, keys, isInner, apiType, message) {
        return this.context.removeRTCData(roomId, keys, isInner, apiType, message);
    }
    setRTCOutData(roomId, rtcData, type, message) {
        return this.context.setRTCOutData(roomId, rtcData, type, message);
    }
    getRTCOutData(roomId, userIds) {
        return this.context.getRTCOutData(roomId, userIds);
    }
    getRTCToken(roomId, mode, broadcastType) {
        return this.context.getRTCToken(roomId, mode, broadcastType);
    }
    setRTCState(roomId, report) {
        return this.context.setRTCState(roomId, report);
    }
    getRTCUserList(roomId) {
        return this.context.getRTCUserList(roomId);
    }
}
...
class Letterbox {
    ...
    setInboxTime(timestamp, userId) {
        if (this.inboxTime > timestamp) {
            return;
        }
        this.inboxTime = timestamp;
        const key = generateKey(INBOX_KEY, this.appkey, userId);
        this.runtime.localStorage.setItem(key, timestamp.toString());
    }
    /**
     * 获取收件箱时间
     * @param userId
    ...
    getInboxTime(userId) {
        if (this.inboxTime === 0) {
            const key = generateKey(INBOX_KEY, this.appkey, userId);
            this.inboxTime = parseInt(this.runtime.localStorage.getItem(key)) || 0;
        }
    ...
    setOutboxTime(timestamp, userId) {
        if (this.outboxTime > timestamp) {
            return;
        }
        this.outboxTime = timestamp;
        const key = generateKey(OUTBOX_KEY, this.appkey, userId);
        this.runtime.localStorage.setItem(key, timestamp.toString());
    }
    /**
     * 获取发件箱时间
    ...
    getOutboxTime(userId) {
        if (this.outboxTime === 0) {
            const key = generateKey(OUTBOX_KEY, this.appkey, userId);
            this.outboxTime = parseInt(this.runtime.localStorage.getItem(key)) || 0;
        }
...
class KVStore {
    ...
    add(kv) {
        const { key } = kv;
        kv.isDeleted = false;
        this.kvCaches[key] = kv;
    }
    remove(kv) {
        const { key } = kv;
        const cacheKV = this.kvCaches[key];
        cacheKV.isDeleted = true;
        this.kvCaches[key] = cacheKV;
    }
    setEntry(data, isFullUpdate) {
        const { key, type, isOverwrite, userId } = data;
        const latestUserId = this.getSetUserId(key);
        const isDeleteOpt = type === ChatroomEntryType$1.DELETE;
        const isSameAtLastSetUser = latestUserId === userId;
        const isKeyNotExist = this.isExisted(key);
        const event = isDeleteOpt ? this.remove : this.add;
        if (isFullUpdate) {
    ...
    getValue(key) {
        const kv = this.kvCaches[key] || {};
        const { isDeleted } = kv;
        return isDeleted ? null : kv.value;
    }
    getAllValue() {
        const entries = {};
        for (const key in this.kvCaches) {
            if (!this.kvCaches[key].isDeleted) {
                entries[key] = this.kvCaches[key].value;
            }
        }
        return entries;
    }
    getSetUserId(key) {
        const cache = this.kvCaches[key] || {};
        return cache.userId;
    }
    isExisted(key) {
        const cache = this.kvCaches[key] || {};
...
class ChrmEntryHandler {
    constructor(engine) {
        ...
        this.storeCaches = {}; // 所有聊天室的 Store 缓存
        this.engine = engine;
    }
    startPull() {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.isPulling || this.pullQueue.length === 0) {
                return;
            }
            this.isPulling = true;
            const { chrmId, timestamp } = this.pullQueue.splice(0, 1)[0];
            const pulledUpTime = PullTimeCache.get(chrmId);
            if (pulledUpTime > timestamp) { // 已经拉取过，不再拉取
                return;
            }
            const { code, data } = yield this.engine.pullChatroomEntry(chrmId, timestamp);
            if (code === ErrorCode$1.SUCCESS) {
                this.isPulling = false;
    ...
    */
    pullEntry(chrmId, timestamp) {
        this.pullQueue.push({ chrmId, timestamp });
        this.startPull();
    }
    /**
     * 向本地缓存己方设置或拉取到的 kv
    ...
    */
    getValue(chrmId, key) {
        // throw new Error('Method not implemented.')
        const kvStore = this.storeCaches[chrmId];
...
class EventDispatcher {
    ...
    on(eventType, listener) {
        const arr = this.map[eventType] || (this.map[eventType] = []);
     ...
     */
    clear() {
        Object.keys(this.map).forEach(this.removeAll, this);
    }
}

const RootStorage = {
    set: (key, val) => {
        runtime$1.localStorage.setItem(key, JSON.stringify(val));
    },
    get: (key) => {
        let val;
        try {
            val = JSON.parse(runtime$1.localStorage.getItem(key));
        }
        catch (e) {
            val = null;
        }
    ...
    remove: (key) => {
        return runtime$1.localStorage.removeItem(key);
    },
    getKeys: () => {
        const keys = [];
        for (const key in runtime$1.localStorage) {
            keys.push(key);
        }
        return keys;
    }
...
class AppStorage {
    constructor(suffix) {
        ...
        const localCache = RootStorage.get(key) || {};
...
class ConversationStatus {
    constructor(engine, appkey, currentUserId) {
        ...
        this.currentUserId = currentUserId;
        this.engine = engine;
        this.storagePullTimeKey = `con-s-${appkey}-${currentUserId}`;
    }
    /**
    ...
    */
    set(list) {
        ...
        let localTime = this.storage.get(this.storagePullTimeKey) || 0;
        ...
        list.forEach((statusItem, index) => {
            ...
            this.eventEmitter.emit(EventName.STATUS_CHANGED, {
                statusItem,
                isLastPull: index === listCount - 1
            });
        });
        this.storage.set(this.storagePullTimeKey, localTime);
    }
    /**
     * 拉取队列
    */
    startPull() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.isPulling = true;
            const time = this.pullQueue.splice(0, 1)[0];
            const { code, data } = yield this.engine.pullConversationStatus(time);
            if (code === ErrorCode$1.SUCCESS) {
                this.isPulling = false;
                this.set(data);
                this.startPull();
    ...
    */
    pull(newPullTime) {
        const time = this.storage.get(this.storagePullTimeKey) || 0;
        if (newPullTime > time || newPullTime === 0) {
            // 拉取,并通知上层拉取到的数据
            this.pullQueue.push(time);
            this.startPull();
        }
    }
    /**
    ...
    */
    watch(event) {
        this.eventEmitter.on(EventName.STATUS_CHANGED, (data) => {
            event(data);
        });
    }
    /**
    ...
    */
    unwatch() {
        this.eventEmitter.off(EventName.STATUS_CHANGED, (data) => { });
    }
}
...
class ConversationStore {
    constructor(appkey, userId) {
        const suffix = `con-${appkey}-${userId}`;
        this.storage = new AppStorage(suffix);
        this.appkey = appkey;
        this.currentUserId = userId;
    }
    getStoreKey(type, targetId) {
        return `${type}_${targetId}`;
    }
    getConOptionByKey(key) {
        key = key || '';
    ...
    */
    updateMentionedData(message) {
        ...
        const key = this.getStoreKey(conversationType, targetId);
        ...
        if (isMentioned && conversationType === ConversationType$1.GROUP && mentionedInfo.userIdList) {
            ...
            receiveUserIdList.forEach(userId => {
                if (userId === this.currentUserId && isNotIncludeSender) {
    ...
    */
    set(type, targetId, conversation) {
        const key = this.getStoreKey(type, targetId);
        const local = this.storage.get(key) || {};
        for (const key in conversation) {
            ...
            if (!local.c) { // 清空未读数则清空最后操作未读时间，避免占用空间
                delete local.t;
            }
        }
        if (notEmptyObject(local)) {
    ...
    */
    getValue(func) {
        ...
        for (const key in values) {
            const { conversationType, targetId } = this.getConOptionByKey(key);
            // TODO 类型待确认
            let conversation = {};
...
class ConversationManager {
    constructor(engine, appkey, userId, updatedConversationFunc) {
        ...
        this.store = new ConversationStore(appkey, userId);
        this.statusManager = new ConversationStatus(engine, appkey, userId);
        this.statusManager.watch((data) => {
            const { statusItem, isLastPull } = data;
            this.addStatus(statusItem, isLastPull);
        });
        this.eventEmitter.on(EventName$1.CHANGED, (data) => {
    ...
    */
    calcUnreadCount(message, localConversation) {
        const { content, messageType, sentTime, isCounted, messageDirection, senderUserId } = message;
        const isSelfSend = messageDirection === MessageDirection$1.SEND && senderUserId === this.loginUserId;
    ...
    */
    calcMentionedInfo(message, localConversation) {
    ...
    */
    setUpdatedConversation(conversation) {
        if (isObject(conversation)) {
            ...
            const cacheConversation = this.updatedConversations[key] || {};
            this.updatedConversations[key] = Object.assign(cacheConversation, conversation);
        }
    }
    addStatus(statusItem, isLastPull) {
        ...
        this.store.set(conversationType, targetId, {
            notificationStatus,
            isTop
        });
     ...
     * 通知会话更新
     * @description
     * 通知的条件: 会话状态变化、会话未读数变化（未读数增加、未读数清空）、会话 @ 信息（hasMentioned、mentionedInfo）、
    */
    notifyConversationChanged() {
        const list = [];
        for (const key in this.updatedConversations) {
            list.push(this.updatedConversations[key]);
        }
        this.eventEmitter.emit(EventName$1.CHANGED, list);
    }
    /**
    ...
    */
    setConversationCacheByMessage(message, isPullMessageFinished) {
        ...
        let hasChanged = false;
        let storageConversation = this.store.get(conversationType, targetId);
        // 计算本地存储
        const CalcEvents = [this.calcUnreadCount, this.calcMentionedInfo];
        ...
        if (hasChanged) {
            this.store.set(conversationType, targetId, storageConversation);
        }
        this.store.updateMentionedData(message);
        // 写入会话缓存中
        if (isPersited) {
            let conversation = this.store.get(conversationType, targetId);
            // TODO 待确认设置原因
            ...
            conversation = Object.assign(conversation, { conversationType, targetId });
            this.setUpdatedConversation(conversation);
        }
    ...
    */
    get(conversationType, targetId) {
        return this.store.get(conversationType, targetId);
    }
    /**
     * 获取本地会话所有未读数
    */
    getAllUnreadCount() {
        const conversationList = this.store.getValue();
        let totalCount = 0;
    ...
    */
    getUnreadCount(conversationType, targetId) {
        const conversation = this.store.get(conversationType, targetId);
    ...
    */
    clearUnreadCount(conversationType, targetId) {
        ...
        this.store.set(conversationType, targetId, conversation);
        conversation = Object.assign(conversation, { conversationType, targetId });
        this.setUpdatedConversation(conversation);
        this.notifyConversationChanged();
    }
    startPullConversationStatus(time) {
        this.statusManager.pull(time);
    }
    /**
     * 设置会话消息草稿
    */
...
class JSEngine extends AEngine {
    constructor(appkey, watcher, _) {
        ...
        this.chrmsQueue = {};
        // 初始化信箱
        this.letterbox = new Letterbox(runtime$1, appkey);
        // 初始化 KV 处理
        this.chrmEntryHandler = new ChrmEntryHandler(this);
    }
    connect(token, naviInfo) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // 创建数据通道
            const channel = runtime$1.createDataChannel({
                status: (status) => {
                    this.connectionStatusHandler(status, token, hosts, naviInfo.protocol);
                },
                signal: this.signalHandler.bind(this)
            });
            // 建立连接
            const code = yield channel.connect(this.appkey, token, hosts, naviInfo.protocol);
            if (code === ErrorCode$1.SUCCESS) {
                this.channel = channel;
                this.currentUserId = channel.userId;
                this.connectedTime = channel.connectedTime;
                this.conversationManager = new ConversationManager(this, this.appkey, this.currentUserId, this.watcher.conversation);
                this.conversationManager.startPullConversationStatus(0);
                // 拉取离线消息
                this.syncMsg();
            }
            else {
    ...
    connectionStatusHandler(status, token, hosts, protocol) {
        ...
        if (!this.channel || status === ConnectionStatus$1.DISCONNECTED) {
            // 用户主动断开连接，直接抛出连接状态
            this.watcher.status(status);
            return;
        }
        ...
        // 异常断开，尝试重连
        this.try2Reconnect(token, hosts, protocol);
    }
    try2Reconnect(token, hosts, protocol) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.channel) {
                return;
            }
            const code = yield this.channel.connect(this.appkey, token, hosts, protocol);
            if (code === ErrorCode$1.SUCCESS) {
                return;
            }
            this.watcher.status(ConnectionStatus$1.WEBSOCKET_UNAVAILABLE);
            // 等待 5s 后重新尝试
            setTimeout(() => {
                this.try2Reconnect(token, hosts, protocol);
            }, 5000);
    ...
    signalHandler(signal, ack) {
        const { syncMsg, topic } = signal;
        if (syncMsg) {
            // 此消息为本人其他端发出的消息，此处为多端消息同步
            this.receiveSyncMsg(signal, ack);
            return;
        }
        ...
        switch (tmpTopic) {
            case Topic$1.s_ntf:
                this.pullMsg(signal); // 通知拉取
                break;
            case Topic$1.s_msg:
                this.receiveMsg(signal); // 接收直发消息
                break;
            case Topic$1.s_cmd:
                this.receiveStateNotify(signal);
                break;
            case Topic$1.s_us:
                this.receiveSettingNotify(signal);
                break;
        }
    }
    /**
     ...
     * @param signal
     */
    receiveStateNotify(signal) {
        var _a;
        const { time, type, chrmId } = (_a = this.channel) === null || _a === void 0 ? void 0 : _a.codec.decodeByPBName(signal.data, PBName.NotifyMsg);
        switch (type) {
            case 2:
                this.chrmEntryHandler.pullEntry(chrmId, time);
                break;
            case 3:
                this.conversationManager.startPullConversationStatus(time);
                break;
        }
    }
    /**
     ...
     * @param signal
     */
    receiveSettingNotify(signal) {
        var _a;
        const notice = (_a = this.channel) === null || _a === void 0 ? void 0 : _a.codec.decodeByPBName(signal.data, PBName.UserSettingNotification);
        logger.error('TODO: 接收用户级配置变更通知', notice);
    }
    /**
     * 通知 API Content 扩展变更
    */
    receiveMessageExpansion(message) {
        const { content } = message;
        const { put, del, mid } = content;
        if (put) {
            this.watcher.expansion({
                updatedExpansion: {
                    messageUId: mid,
                    expansion: put
                }
     ...
     */
    receiveSyncMsg(signal, ack) {
        var _a;
        const msg = (_a = this.channel) === null || _a === void 0 ? void 0 : _a.codec.decodeByPBName(signal.data, PBName.UpStreamMessage, {
        ...
        if (!this.pullingMsg) {
            this.pullQueue.push(ack.timestamp);
            return;
        }
        // 更新发件箱时间
        this.letterbox.setOutboxTime(ack.timestamp, this.currentUserId);
        if (msg.messageType === MessageType$1.EXPANSION_NOTIFY) {
            this.receiveMessageExpansion(msg);
            return;
        }
        this.watcher.message(msg);
    }
    /**
     * 拉取消息
    ...
    pullMsg(signal) {
        ...
        const { type, chrmId, time } = this.channel.codec.decodeByPBName(signal.data, PBName.NotifyMsg);
        if (type === 2) {
            const info = this.chrmsQueue[chrmId];
            info.queue.push(time);
            this.pullChrmMsg(chrmId);
        }
        else {
            // 记录消息拉取通知的时间戳
            this.pullQueue.push(time);
    ...
    syncMsg() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // 获取消息时间戳
            const outboxTime = this.letterbox.getOutboxTime(this.currentUserId);
            const inboxTime = this.letterbox.getInboxTime(this.currentUserId);
            logger.warn('outboxTime', outboxTime);
            logger.warn('inboxTime', inboxTime);
            const reqBody = this.channel.codec.encodeSyncMsg({ sendboxTime: outboxTime, inboxTime });
            const writer = new QueryWriter(Topic$1[Topic$1.pullMsg], reqBody, this.currentUserId);
            const { code, data } = yield this.channel.send(writer, PBName.DownStreamMessages, {
                connectedTime: this.channel.connectedTime,
            ...
            list.forEach(item => {
                ...
                if (item.messageType === MessageType$1.EXPANSION_NOTIFY) {
                    this.receiveMessageExpansion(item);
                    return;
                }
                this.watcher.message(item);
                this.conversationManager.setConversationCacheByMessage(item, true);
            });
            // 更新收件箱时间
            // this.letterbox.setInboxTime(newInboxTime, this.currentUserId)
            this.letterbox.setInboxTime(syncTime, this.currentUserId);
            // 更新发件箱时间
            this.letterbox.setOutboxTime(newOutboxTime, this.currentUserId);
            this.pullingMsg = false;
            // 清除较 syncTime 更早的拉取通知时间戳
            const tmpPullQueue = this.pullQueue.filter(timestamp => timestamp > syncTime);
            this.pullQueue.length = 0;
            this.pullQueue.push(...tmpPullQueue);
    ...
    receiveMsg(signal) {
        ...
        const msg = this.channel.codec.decodeByPBName(signal.data, PBName.DownStreamMessage, {
            currentUserId: this.currentUserId, connectedTime: this.channel.connectedTime
        });
        ...
        // 更新收件箱时间
        this.letterbox.setInboxTime(msg.sentTime, this.currentUserId);
        if (msg.messageType === MessageType$1.EXPANSION_NOTIFY) {
            this.receiveMessageExpansion(msg);
            return;
        }
        this.watcher.message(msg);
        this.conversationManager.setConversationCacheByMessage(msg, true);
    }
    getConnectTime() {
        if (this.channel) {
            return Promise.resolve({
                code: ErrorCode$1.SUCCESS,
                data: this.channel.connectedTime
            });
        }
    ...
    getHistoryMessage(conversationType, targetId, timestamp, count, order) {
        return __awaiter(this, void 0, void 0, function* () {
            const { currentUserId, channel } = this;
    ...
    getConversationList(count = 300, conversationType) {
        return __awaiter(this, void 0, void 0, function* () {
            const { currentUserId, channel } = this;
            conversationType = conversationType || ConversationType$1.PRIVATE;
            if (channel) {
                const buff = channel.codec.encodeOldConversationList({ count, type: conversationType });
                ...
                const resp = yield channel.send(writer, PBName.RelationsOutput, {
                    ...
                    afterDecode: (conversation) => {
                        const { conversationType, targetId } = conversation;
                        const localConversation = this.conversationManager.get(conversationType, targetId);
    ...
    getAllConversationUnreadCount() {
        const allUnreadCount = this.conversationManager.getAllUnreadCount();
        return Promise.resolve({
            code: ErrorCode$1.SUCCESS,
            data: allUnreadCount
        });
    ...
    getConversationUnreadCount(conversationType, targetId) {
        const unreadCount = this.conversationManager.getUnreadCount(conversationType, targetId);
        return Promise.resolve({
            code: ErrorCode$1.SUCCESS,
            data: unreadCount
        });
    ...
    clearConversationUnreadCount(conversationType, targetId) {
        this.conversationManager.clearUnreadCount(conversationType, targetId);
        return Promise.resolve(ErrorCode$1.SUCCESS);
    }
    saveConversationMessageDraft(conversationType, targetId, draft) {
        this.conversationManager.setDraft(conversationType, targetId, draft);
        return Promise.resolve(ErrorCode$1.SUCCESS);
    }
    getConversationMessageDraft(conversationType, targetId) {
        const draft = this.conversationManager.getDraft(conversationType, targetId);
        return Promise.resolve({
            code: ErrorCode$1.SUCCESS,
            data: draft
        });
    ...
    clearConversationMessageDraft(conversationType, targetId) {
        this.conversationManager.clearDraft(conversationType, targetId);
        return Promise.resolve(ErrorCode$1.SUCCESS);
    }
    pullConversationStatus(timestamp) {
        return __awaiter(this, void 0, void 0, function* () {
            const { channel, currentUserId } = this;
            if (channel) {
                ...
                logger.warn('pull conversation status data >>>>>>>>>\n', buff, writer);
            ...
            if (channel) {
                ...
                if (code === ErrorCode$1.SUCCESS) {
                    ...
                    statusList.forEach((item) => {
                        this.conversationManager.addStatus(Object.assign(Object.assign({}, item), { updatedTime: versionData.version }), true);
                    });
                    return code;
                }
                return code;
    ...
    joinChrm(chrmId, count, isJoinExist) {
        return __awaiter(this, void 0, void 0, function* () {
            const { channel } = this;
            if (!channel)
                return ErrorCode$1.RC_NET_CHANNEL_INVALID;
            ...
            if (code === ErrorCode$1.SUCCESS) {
                ...
                if (!info) {
                    this.chrmsQueue[chrmId] = { pulling: false, queue: [], timestamp: 0 };
                }
                this.pullChrmMsg(chrmId, count);
                // 如果开通聊天室 KV 存储服务, 加入成功后拉取聊天室 KV 存储
                const { kvStorage: isOpenKVService } = this.naviInfo;
                if (isOpenKVService) {
                    this.chrmEntryHandler.pullEntry(chrmId, 0);
                }
            }
            return code;
        });
     ...
     */
    pullChrmMsg(chrmId, count = 10) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.channel) {
                return;
            }
            const chrmInfo = this.chrmsQueue[chrmId];
            const { pulling, timestamp } = chrmInfo;
            if (pulling) {
                return;
            }
            const reqBody = this.channel.codec.encodeChrmSyncMsg(timestamp, count);
            const signal = new QueryWriter(Topic$1[Topic$1.chrmPull], reqBody, chrmId);
            const { code, data } = yield this.channel.send(signal, PBName.DownStreamMessages, {
                connectedTime: this.channel.connectedTime,
                currentUserId: this.currentUserId
            });
            if (code !== ErrorCode$1.SUCCESS || !data) {
    ...
    joinChatroom(chatroomId, count) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.joinChrm(chatroomId, count, false);
        });
    }
    joinExistChatroom(chatroomId, count) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.joinChrm(chatroomId, count, true);
        });
    }
    quitChatroom(chrmId) {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    modifyChatroomKV(chatroomId, entry) {
        return __awaiter(this, void 0, void 0, function* () {
            const { channel, currentUserId } = this;
            if (!channel)
                return ErrorCode$1.RC_NET_CHANNEL_INVALID;
            ...
            const { code } = resp;
            if (code === ErrorCode$1.SUCCESS) {
                this.chrmEntryHandler.setLocal(chatroomId, {
                    kvEntries: [entry],
    ...
    setChatroomEntry(chatroomId, entry) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return this.modifyChatroomKV(chatroomId, entry);
        });
    }
    ...
    getChatroomEntry(chatroomId, key) {
        ...
        const entry = this.chrmEntryHandler.getValue(chatroomId, key);
        if (entry) {
            return Promise.resolve({
                code: ErrorCode$1.SUCCESS,
                data: entry
    ...
    getAllChatroomEntry(chatroomId) {
        // 1、判断用户是否在聊天室，不在抛出 不在聊天室 错误码 2、从本地获取 key value 属性
        const entries = this.chrmEntryHandler.getAll(chatroomId);
        return Promise.resolve({
            code: ErrorCode$1.SUCCESS,
            data: entries
        });
    ...
    */
    pullChatroomEntry(chatroomId, timestamp) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { code, data } = resp;
            logger.debug('pullChatroomEntry', resp);
            if (code === ErrorCode$1.SUCCESS) {
                // 拉取完成后，向本地缓存 kv
                this.chrmEntryHandler.setLocal(chatroomId, data, currentUserId);
                ...
                if (kvEntries.length > 0) {
                    ...
                    this.watcher.chatroom({ updatedEntries });
                }
                return {
                    code,
                    data: data
    ...
    recallMsg(conversationType, targetId, messageUId, sentTime, user) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const data = this.channel.codec.encodeUpMsg({ type: conversationType, targetId }, msg);
            const signal = new PublishWriter(topic, data, targetId);
            signal.setHeaderQos(QOS.AT_LEAST_ONCE);
            const { code, data: resp } = yield this.channel.send(signal);
            if (code !== ErrorCode$1.SUCCESS) {
                return { code };
    ...
    pullUserSettings(version) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const buff = this.channel.codec.encodePullUserSetting(version);
            const writer = new QueryWriter(Topic$1[Topic$1.pullUS], buff, this.currentUserId);
            return this.channel.send(writer, PBName.PullUserSettingOutput);
        });
    }
    getFileToken(fileType, fileName) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const buff = this.channel.codec.encodeGetFileToken(fileType, uploadFileName);
            const writer = new QueryWriter(Topic$1[Topic$1.qnTkn], buff, this.currentUserId);
            const { code, data } = yield this.channel.send(writer, PBName.GetQNupTokenOutput);
            if (code === ErrorCode$1.SUCCESS) {
                return {
                    code,
                    data: data
    ...
    getFileUrl(fileType, fileName, originName) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const buff = this.channel.codec.encodeGetFileUrl(fileType, fileName, originName);
            const writer = new QueryWriter(Topic$1[Topic$1.qnUrl], buff, this.currentUserId);
            const { code, data } = yield this.channel.send(writer, PBName.GetQNdownloadUrlOutput);
            const resp = data;
            if (code === ErrorCode$1.SUCCESS) {
                return {
                    code,
    ...
    joinRTCRoom(roomId, mode, broadcastType) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const reqBody = this.channel.codec.encodeJoinRTCRoom(mode, broadcastType);
            const writer = new QueryWriter(Topic$1[Topic$1.rtcRJoin_data], reqBody, roomId);
            return this.channel.send(writer, PBName.RtcUserListOutput);
        });
    }
    quitRTCRoom(roomId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const reqBody = this.channel.codec.encodeQuitRTCRoom();
            const writer = new QueryWriter(Topic$1[Topic$1.rtcRExit], reqBody, roomId);
            const { code } = yield this.channel.send(writer);
            return code;
        });
    }
    rtcPing(roomId, mode, broadcastType) {
    ...
    getRTCRoomInfo(roomId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const reqBody = this.channel.codec.encodeGetRTCRoomInfo();
            const writer = new QueryWriter(Topic$1[Topic$1.rtcRInfo], reqBody, roomId);
            return this.channel.send(writer, PBName.RtcRoomInfoOutput);
        });
    }
    getRTCUserInfoList(roomId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { code, data } = yield this.channel.send(writer, PBName.RtcUserListOutput);
    ...
    setRTCUserInfo(roomId, key, value) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const reqBody = this.channel.codec.encodeSetRTCUserInfo(key, value);
    ...
    removeRTCUserInfo(roomId, keys) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const reqBody = this.channel.codec.encodeRemoveRTCUserInfo(keys);
    ...
    setRTCData(roomId, key, value, isInner, apiType, message) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const reqBody = this.channel.codec.encodeSetRTCData(key, value, isInner, apiType, message);
    ...
    setRTCTotalRes(roomId, message, valueInfo, objectName) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const reqBody = this.channel.codec.encodeUserSetRTCData(message, valueInfo, objectName);
    ...
    getRTCData(roomId, keys, isInner, apiType) {
        ...
        const reqBody = this.channel.codec.encodeGetRTCData(keys, isInner, apiType);
        const writer = new QueryWriter(Topic$1[Topic$1.rtcQryData], reqBody, roomId);
        return this.channel.send(writer, PBName.RtcQryOutput);
    }
    removeRTCData(roomId, keys, isInner, apiType, message) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.channel) {
            ...
            const reqBody = this.channel.codec.encodeRemoveRTCData(keys, isInner, apiType, message);
    ...
    getRTCToken(roomId, mode, broadcastType) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return this.channel.send(writer, PBName.RtcTokenOutput);
        });
    }
    setRTCState(roomId, report) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const reqBody = this.channel.codec.encodeSetRTCState(report);
    ...
    getRTCUserList(roomId) {
        ...
        const data = this.channel.codec.encodeGetRTCRoomInfo();
...
class APIContext {
     ...
     * 7. 公有云默认使用 https 协议
     */
    navigators = [], cppProtocol, customCMP) {
        this.appkey = appkey;
        ...
        this.pluginQueue = [];
        /**
         * 对外的 plugins 挂载点，预定义 rtc\call\live 的接口，便于用户代码集成时提供代码提示
         */
        this.pluginApi = {};
        /**
         * SDK 版本号
         */
        this.apiVersion = "3.1.0";
        /**
         * 核心库版本号，后期与 4.0 IM SDK 版本号保持一致
         */
        this.coreVersion = "4.0.0";
        this.connectionStatus = ConnectionStatus$1.DISCONNECTED;
        /**
         * 业务层事件监听器挂载点
         */
        this.watcher = {
        ...
        if (typeof appkey !== 'string' || appkey.length === 0) {
            const msg = 'appkey is required!';
            logger.error(msg);
            throw new Error(msg);
        }
        // 过滤无效地址
        navigators = navigators.filter(item => /^https?:\/\//.test(item));
        // 有自定义导航的状态下，不再使用内置导航地址
        if (navigators.length === 0) {
            const navis =  PUBLIC_CLOUD_NAVI_URIS;
            navigators.push(...navis);
        }
        const plugins = APIContext.pluginInstallers;
        if (plugins.length) {
            // 初始化 plugin
            logger.debug('init plugin context');
            const pluginContext = new PluginContext(this);
            logger.debug('install plugins =>', ...plugins.map(item => `${item.tag}-${item.version}`));
            plugins.forEach(installer => {
                // RTC 需求的信令内容与信令服务紧耦合，故此部分内容不可拆分
                const plugin = installer.setup(installer.tag === 'RTC' ? pluginContext : new RTCPluginContext(this), runtime$1);
                // 加入队列
                this.pluginQueue.push(plugin);
                // 挂载对外接口
                this.pluginApi[installer.tag] = plugin.getAPIInterface();
            });
            logger.debug('plugins installed');
        }
        // 初始化 Navi 模块
        this.navi = new Navi(appkey, navigators, customCMP);
        // 初始化引擎监听器，监听连接状态变化、消息变化以及聊天室状态变化
        const engineWatcher = {
            status: this.connectionStatusListener.bind(this),
            message: this.messageReceiver.bind(this),
            chatroom: this.chatroomInfoListener.bind(this),
            conversation: this.conversationInfoListener.bind(this),
            expansion: this.expansionInfoListener.bind(this)
        };
        // 初始化引擎
        this.engine =  new JSEngine(appkey, engineWatcher);
        // 初始化完成，插件钩子触发
        this.pluginQueue.forEach(item => item.onInit && item.onInit(appkey));
    }
    /**
     * 集成插件，需在 init 之前调用
     * @param installer
     */
    static use(...installer) {
        if (this.context) {
            logger.error('have to call method `use` before `init`');
            return;
        }
        if (!installer || installer.length === 0) {
            logger.warn('miss params');
            return;
        }
        installer.forEach(item => this.pluginInstallers.includes(item) || this.pluginInstallers.push(item));
    }
    /**
     * 初始化
     * @param appkey
     * @param debug
     * @param navigators 自定义导航
     * @param cppProtocol c++ 协议栈
     */
    static init(appkey, debug, navigators, cppProtocol, customCMP) {
        logger.debug('APIContext.init =>', appkey, debug, navigators);
        if (this.context) {
            logger.warn('Repeat initialize!');
            return this.context;
        }
        this.context = new APIContext(appkey, navigators, cppProtocol, customCMP);
        return this.context;
    }
    static destroy() {
        if (this.context) {
    ...
    connectionStatusListener(status) {
        ...
        ((_a = this.watcher.rtcInnerWatcher) === null || _a === void 0 ? void 0 : _a.status) && this.watcher.rtcInnerWatcher.status(status);
        // 通知插件连接状态变更
        this.pluginQueue.forEach(item => item.onConnectionStatusChanged && item.onConnectionStatusChanged(status));
        // 通知应用层连接状态变更
        this.watcher.connectionState && this.watcher.connectionState(status);
    }
    messageReceiver(message) {
        ...
        if (message.conversationType === ConversationType$1.RTC_ROOM ||
             ...
             */
            if (this.watcher.rtcInnerWatcher && this.watcher.rtcInnerWatcher.message) {
                this.watcher.rtcInnerWatcher.message(message);
                return;
            }
        }
        for (let i = 0, len = this.pluginQueue.length; i < len; i += 1) {
            const plugin = this.pluginQueue[i];
            const done = plugin.onReceiveMessage && plugin.onReceiveMessage(cloneMessage(message));
            if (done) {
                return;
            }
        }
        // 最终未被过滤的消息派发给应用层
        this.watcher.message && this.watcher.message(cloneMessage(message));
    }
    /**
     * 聊天室相关信息监听
    */
    chatroomInfoListener(info) {
        this.watcher.chatroomState && this.watcher.chatroomState(info);
    }
    /**
     * 会话监听相关
    */
    conversationInfoListener(info) {
        this.watcher.conversationState && this.watcher.conversationState(info);
    }
    /**
     * 消息扩展监听相关
    */
    expansionInfoListener(info) {
        this.watcher.expansion && this.watcher.expansion(info);
    }
    /**
     * 添加事件监听
     * @param options
    ...
    assignWatcher(watcher) {
        ...
        Object.keys(this.watcher).forEach((key) => {
            if (Object.prototype.hasOwnProperty.call(watcher, key)) {
                ...
                this.watcher[key] = isFunction(value) || isObject(value) ? value : undefined;
            }
        });
    }
    getConnectedTime() {
        return this.engine.connectedTime;
    }
    getCurrentUserId() {
        return this.engine.currentUserId;
    }
    getConnectionStatus() {
    ...
    connect(token, refreshNavi = false) {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.connectionStatus === ConnectionStatus$1.CONNECTED) {
                return { code: ErrorCode$1.SUCCESS, userId: this.engine.currentUserId };
            }
            if (this.connectionStatus === ConnectionStatus$1.CONNECTING) {
            ...
            // 获取导航数据
            const naviInfo = yield this.navi.getInfo(this.getTokenWithoutNavi(), dynamicUris, refreshNavi);
            if (!naviInfo) {
                return { code: ErrorCode$1.RC_NAVI_RESOURCE_ERROR };
            }
            // 开始连接，并监听链接状态变化，状态为 0 则连接成功
            const code = yield this.engine.connect(this.getTokenWithoutNavi(), naviInfo);
            if (code === ErrorCode$1.SUCCESS) {
                // 拉取用户级配置
                naviInfo.openUS === 1 && this.pullUserSettings();
            }
            return { code, userId: naviInfo.userId };
        });
    }
    getConnectTime() {
        return this.engine.getConnectTime();
    }
    /**
    ...
    pullUserSettings() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const version = 0;
            const res = yield this.engine.pullUserSettings(version);
    ...
    reconnect() {
        return this.connect(this.getTokenWithoutNavi());
    }
    // 获取 token 动态导航前的部分
    getTokenWithoutNavi() {
     ...
     */
    getInfoFromCache() {
        return this.navi.getInfoFromCache(this.getTokenWithoutNavi());
    }
    /**
     * 消息注册
     * @description 消息注册需在应用初始化完成前进行
     ...
     */
    registerMessageType(objectName, isPersited, isCounted, searchProps = []) {
        this.engine.registerMessageType(objectName, isPersited, isCounted, searchProps);
    }
    /**
     * 发送消息
     * @param conversationType
    ...
    sendMessage(conversationType, targetId, options) {
        ...
        return this.engine.sendMessage(conversationType, targetId, options);
    }
    /**
     * 发送扩展消息
    ...
    */
    sendExpansionMessage(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (isObject(expansion)) {
                ...
                const totalExpansion = Object.assign(expansion, originExpansion);
            ...
            const { code } = yield this.engine.sendMessage(conversationType, targetId, {
                content,
    ...
    getHistoryMessage(conversationType, targetId, timestamp = 0, count = 20, order = 0) {
        return this.engine.getHistoryMessage(conversationType, targetId, timestamp, count, order);
    }
    /**
     * 获取会话列表
     ...
     */
    getConversationList(count = 300, conversationType) {
        return this.engine.getConversationList(count, conversationType);
    }
    /**
     * 删除会话
     */
    removeConversation(conversationType, targetId) {
        return this.engine.removeConversation(conversationType, targetId);
    }
    /**
     * 清除会话消息未读数
     */
    clearUnreadCount(conversationType, targetId) {
        return this.engine.clearConversationUnreadCount(conversationType, targetId);
    }
    /**
     * 获取指定会话消息未读数
     */
    getUnreadCount(conversationType, targetId) {
        return this.engine.getConversationUnreadCount(conversationType, targetId);
    }
    getTotalUnreadCount() {
        return this.engine.getAllConversationUnreadCount();
    }
    setConversationStatus(conversationType, targetId, isTop, notificationStatus) {
        ...
        return this.engine.batchSetConversationStatus(statusList);
    }
    saveConversationMessageDraft(conversationType, targetId, draft) {
        return this.engine.saveConversationMessageDraft(conversationType, targetId, draft);
    }
    getConversationMessageDraft(conversationType, targetId) {
        return this.engine.getConversationMessageDraft(conversationType, targetId);
    }
    clearConversationMessageDraft(conversationType, targetId) {
        return this.engine.clearConversationMessageDraft(conversationType, targetId);
    }
    recallMessage(conversationType, targetId, messageUId, sentTime, user) {
        return this.engine.recallMsg(conversationType, targetId, messageUId, sentTime, user);
    }
    /**
     * 删除远端消息
     * @param conversationType
     ...
     */
    deleteRemoteMessage(conversationType, targetId, list) {
        return this.engine.deleteRemoteMessage(conversationType, targetId, list);
    }
    /**
     * 根据时间戳删除指定时间之前的
     * @param conversationType
     ...
     */
    deleteRemoteMessageByTimestamp(conversationType, targetId, timestamp) {
        return this.engine.deleteRemoteMessageByTimestamp(conversationType, targetId, timestamp);
    }
    /**
     * 加入聊天室，若聊天室不存在则创建聊天室
    ...
    joinChatroom(roomId, count = 10) {
        return this.engine.joinChatroom(roomId, count);
    }
    /**
     * 加入聊天室，若聊天室不存在则抛出异常
     * @param roomId 聊天室房间 Id
    ...
    joinExistChatroom(roomId, count = 10) {
        return this.engine.joinExistChatroom(roomId, count);
    }
    /**
     * 退出聊天室
     * @param roomId
     */
    quitChatroom(roomId) {
        return this.engine.quitChatroom(roomId);
    }
    /**
     * 获取聊天室房间数据
    ...
    getChatroomInfo(roomId, count = 0, order = 0) {
        return this.engine.getChatroomInfo(roomId, count, order);
    }
    /**
     * 在指定聊天室中设置自定义属性
    ...
    setChatroomEntry(roomId, entry) {
        return this.engine.setChatroomEntry(roomId, entry);
    }
    /**
    ...
    forceSetChatroomEntry(roomId, entry) {
        return this.engine.forceSetChatroomEntry(roomId, entry);
    }
    /**
     * 删除聊天室属性
    ...
    removeChatroomEntry(roomId, entry) {
        return this.engine.removeChatroomEntry(roomId, entry);
    }
    /**
     * 强制删除任意聊天室属性
     * @description 该方法仅限于删除自己设置的聊天室属性
    ...
    forceRemoveChatroomEntry(roomId, entry) {
        return this.engine.forceRemoveChatroomEntry(roomId, entry);
    }
    /**
     * 获取聊天室中的指定属性
     * @param roomId 聊天室房间 id
    ...
    getChatroomEntry(roomId, key) {
        return this.engine.getChatroomEntry(roomId, key);
    }
    /**
     * 获取聊天室内的所有属性
     * @param roomId 聊天室房间 id
    ...
    getAllChatroomEntries(roomId) {
        return this.engine.getAllChatroomEntry(roomId);
    }
    /**
     * 拉取聊天室内的历史消息
     * @param roomId
    ...
    getChatRoomHistoryMessages(roomId, count = 20, order = 0, timestamp = 0) {
        return this.engine.getChatroomHistoryMessages(roomId, timestamp, count, order);
    }
    /**
     * 获取 七牛、百度上传认证信息
     * @param fileType 文件类型
    ...
    */
    getFileToken(fileType, fileName) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { code, data } = yield this.engine.getFileToken(fileType, fileName);
            if (code === ErrorCode$1.SUCCESS) {
    ...
    */
    getFileUrl(fileType, fileName, originName, uploadRes) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { code, data } = yield this.engine.getFileUrl(fileType, fileName, originName);
            if (code === ErrorCode$1.SUCCESS) {
     ...
     */
    joinRTCRoom(roomId, mode, broadcastType) {
        return this.engine.joinRTCRoom(roomId, mode, broadcastType);
    }
    quitRTCRoom(roomId) {
        return this.engine.quitRTCRoom(roomId);
    }
    rtcPing(roomId, mode, broadcastType) {
        return this.engine.rtcPing(roomId, mode, broadcastType);
    }
    getRTCRoomInfo(roomId) {
        return this.engine.getRTCRoomInfo(roomId);
    }
    getRTCUserInfoList(roomId) {
        return this.engine.getRTCUserInfoList(roomId);
    }
    getRTCUserInfo(roomId) {
        return this.engine.getRTCUserInfo(roomId);
    }
    setRTCUserInfo(roomId, key, value) {
        return this.engine.setRTCUserInfo(roomId, key, value);
    }
    removeRTCUserInfo(roomId, keys) {
        return this.engine.removeRTCUserInfo(roomId, keys);
    }
    setRTCData(roomId, key, value, isInner, apiType, message) {
        return this.engine.setRTCData(roomId, key, value, isInner, apiType, message);
    }
    setRTCTotalRes(roomId, message, valueInfo, objectName) {
        return this.engine.setRTCTotalRes(roomId, message, valueInfo, objectName);
    }
    getRTCData(roomId, keys, isInner, apiType) {
        return this.engine.getRTCData(roomId, keys, isInner, apiType);
    }
    removeRTCData(roomId, keys, isInner, apiType, message) {
        return this.engine.removeRTCData(roomId, keys, isInner, apiType, message);
    }
    setRTCOutData(roomId, rtcData, type, message) {
        return this.engine.setRTCOutData(roomId, rtcData, type, message);
    }
    getRTCOutData(roomId, userIds) {
        return this.engine.getRTCOutData(roomId, userIds);
    }
    getRTCToken(roomId, mode, broadcastType) {
        return this.engine.getRTCToken(roomId, mode, broadcastType);
    }
    setRTCState(roomId, report) {
        return  this.engine.setRTCState(roomId, report) ;
    }
    getRTCUserList(roomId) {
        return this.engine.getRTCUserList(roomId);
    }
}
APIContext.pluginInstallers = [];
...
(function (AssertRules) {
     ...
     * 类型为数字
     */
    AssertRules[AssertRules["NUMBER"] = 1] = "NUMBER";
    /**
     * 类型为布尔值
     */
    AssertRules[AssertRules["BOOLEAN"] = 2] = "BOOLEAN";
    /**
     * 类型为对象
    */
    AssertRules[AssertRules["OBJECT"] = 3] = "OBJECT";
    /**
     * 类型为数组
    */
    AssertRules[AssertRules["ARRAY"] = 4] = "ARRAY";
...
function tranReceivedMessage(msg) {
    const { conversationType: type, messageType, content, senderUserId, targetId, sentTime, receivedTime, messageUId, messageDirection, isPersited, isCounted, isOffLineMessage, canIncludeExpansion, expansion } = msg;
    return {
        messageType,
        content,
        senderUserId,
...
function tranReceiveUpdateConversation(conversation) {
    const { updatedItems, conversationType: type, targetId, latestMessage, unreadMessageCount, lastUnreadTime, notificationStatus, isTop } = conversation;
    const latestMessageV3 = latestMessage && tranReceivedMessage(latestMessage);
    return {
        updatedItems,
        type,
...
class Conversation {
    ...
    destory() {
        return __awaiter(this, void 0, void 0, function* () {
            const code = yield this.context.removeConversation(this.type, this.targetId);
            if (code !== ErrorCode$1.SUCCESS) {
    ...
    read() {
        return __awaiter(this, void 0, void 0, function* () {
            const code = yield this.context.clearUnreadCount(this.type, this.targetId);
            if (code !== ErrorCode$1.SUCCESS) {
                return Promise.reject({ code, msg: ERROR_CODE[code] });
            }
        });
    ...
    getUnreadCount() {
        return __awaiter(this, void 0, void 0, function* () {
            const { code, data } = yield this.context.getUnreadCount(this.type, this.targetId);
    ...
    send(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { code, data } = yield this.context.sendMessage(this.type, this.targetId, options);
            if (code === ErrorCode$1.SUCCESS) {
    ...
    setStatus(status) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const code = yield this.context.setConversationStatus(this.type, this.targetId, status.isTop, status.notificationStatus);
            if (code !== ErrorCode$1.SUCCESS) {
                return Promise.reject({ code, msg: ERROR_CODE[code] });
            }
        });
    ...
    getMessages(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { code, data } = yield this.context.getHistoryMessage(this.type, this.targetId, options === null || options === void 0 ? void 0 : options.timestamp, options === null || options === void 0 ? void 0 : options.count, options === null || options === void 0 ? void 0 : options.order);
            if (code === ErrorCode$1.SUCCESS && data) {
    ...
    recall(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { code, data } = yield this.context.recallMessage(this.type, this.targetId, options.messageUId, options.sentTime, options.user);
            if (code === ErrorCode$1.SUCCESS && data) {
                return tranReceivedMessage(data);
            }
            return Promise.reject({ code, msg: ERROR_CODE[code] });
    ...
    deleteMessages(messages) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const code = yield this.context.deleteRemoteMessage(this.type, this.targetId, messages);
            if (code !== ErrorCode$1.SUCCESS) {
                return Promise.reject({ code, msg: ERROR_CODE[code] });
            }
        });
    ...
    clearMessages(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const code = yield this.context.deleteRemoteMessageByTimestamp(this.type, this.targetId, options.timestamp);
            if (code !== ErrorCode$1.SUCCESS) {
                return Promise.reject({ code, msg: ERROR_CODE[code] });
            }
        });
    ...
    */
    updateMessageExpansion(expansion, message) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { code } = yield this.context.sendExpansionMessage({
                conversationType,
                targetId,
                messageUId,
                expansion,
    ...
    */
    setDraft(conversationType, targetId, draft) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const code = yield this.context.saveConversationMessageDraft(conversationType, targetId, draft);
            if (code === ErrorCode$1.SUCCESS) {
                return Promise.resolve();
            }
        });
    ...
    */
    getDraft(conversationType, targetId) {
        return __awaiter(this, void 0, void 0, function* () {
            assert('conversationType', conversationType, AssertRules.NUMBER, true);
            assert('targetId', targetId, AssertRules.STRING, true);
            const { code, data } = yield this.context.getConversationMessageDraft(conversationType, targetId);
            if (code === ErrorCode$1.SUCCESS) {
                return Promise.resolve(data);
            }
            return Promise.reject({ code, msg: ERROR_CODE[code] });
     ...
     * 删除会话文本草稿
    */
    deleteDraft(conversationType, targetId) {
        return __awaiter(this, void 0, void 0, function* () {
            assert('conversationType', conversationType, AssertRules.NUMBER, true);
            assert('targetId', targetId, AssertRules.STRING, true);
            const code = yield this.context.clearConversationMessageDraft(conversationType, targetId);
            if (code === ErrorCode$1.SUCCESS) {
                return Promise.resolve();
            }
        });
...
class Chatroom {
    ...
    join(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const code = yield this.context.joinChatroom(this.id, options.count);
            if (code !== ErrorCode$1.SUCCESS) {
                return Promise.reject({ code, msg: ERROR_CODE[code] });
            }
        });
    ...
    joinExist(options) {
        return __awaiter(this, void 0, void 0, function* () {
            assert('options.count', options.count, AssertRules.NUMBER, true);
            const code = yield this.context.joinExistChatroom(this.id, options.count);
            if (code !== ErrorCode$1.SUCCESS) {
                return Promise.reject({ code, msg: ERROR_CODE[code] });
            }
        });
    ...
    quit() {
        return __awaiter(this, void 0, void 0, function* () {
            const code = yield this.context.quitChatroom(this.id);
            if (code !== ErrorCode$1.SUCCESS) {
                return Promise.reject({ code, msg: ERROR_CODE[code] });
            }
        });
    ...
    getInfo(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { code, data: chatroomInfo } = yield this.context.getChatroomInfo(this.id, options.count, options.order);
    ...
    setEntry(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const code = yield this.context.setChatroomEntry(this.id, options);
            if (code !== ErrorCode$1.SUCCESS) {
                return Promise.reject({ code, msg: ERROR_CODE[code] });
            }
        });
    ...
    forceSetEntry(options) {
        return __awaiter(this, void 0, void 0, function* () {
            assertSetChatRoomEntryOption(options);
            const code = yield this.context.forceSetChatroomEntry(this.id, options);
            if (code !== ErrorCode$1.SUCCESS) {
                return Promise.reject({ code, msg: ERROR_CODE[code] });
            }
        });
    ...
    removeEntry(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const code = yield this.context.removeChatroomEntry(this.id, options);
            if (code !== ErrorCode$1.SUCCESS) {
                return Promise.reject({ code, msg: ERROR_CODE[code] });
            }
        });
    ...
    forceRemoveEntry(options) {
        return __awaiter(this, void 0, void 0, function* () {
            assertRemoveChatRoomEntryOption(options);
            const code = yield this.context.forceRemoveChatroomEntry(this.id, options);
            if (code !== ErrorCode$1.SUCCESS) {
                return Promise.reject({ code, msg: ERROR_CODE[code] });
            }
        });
     ...
     */
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { code, data } = yield this.context.getChatroomEntry(this.id, key);
            if (code === ErrorCode$1.SUCCESS && data) {
                return data;
            }
            return Promise.reject({ code, msg: ERROR_CODE[code] });
    ...
    getAllEntries() {
        return __awaiter(this, void 0, void 0, function* () {
            const { code, data } = yield this.context.getAllChatroomEntries(this.id);
            if (code === ErrorCode$1.SUCCESS && data) {
                return data;
            }
            return Promise.reject({ code, msg: ERROR_CODE[code] });
    ...
    send(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { code, data } = yield this.context.sendMessage(ConversationType$1.CHATROOM, this.id, options);
            if (code === ErrorCode$1.SUCCESS) {
                return tranReceivedMessage(data);
            }
            return Promise.reject({ code, msg: ERROR_CODE[code] });
    ...
    getMessages(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { code, data } = yield this.context.getChatRoomHistoryMessages(this.id, options.count, options.order, options.timestamp);
            if (code === ErrorCode$1.SUCCESS && data) {
                const list = data.list.map(item => tranReceivedMessage(item));
                return {
                    list,
    ...
    */
    recall(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { code, data } = yield this.context.recallMessage(conversationType, this.id, options.messageUId, options.sentTime, options.user);
            if (code === ErrorCode$1.SUCCESS && data) {
                return tranReceivedMessage(data);
            }
            return Promise.reject({ code, msg: ERROR_CODE[code] });
...
class RTCClient {
    constructor(_options, _context) {
        ...
        this.roomId = _options.id;
    }
    join() {
        return __awaiter(this, void 0, void 0, function* () {
            const { code, data } = yield this._context.joinRTCRoom(this.roomId, this._options.mode, this._options.broadcastType);
            if (code === ErrorCode$1.SUCCESS) {
                return data;
            }
            return Promise.reject(code);
    ...
    quit() {
        return __awaiter(this, void 0, void 0, function* () {
            const code = yield this._context.quitRTCRoom(this.roomId);
            return code === ErrorCode$1.SUCCESS ? code : Promise.reject(code);
        });
    }
    getRoomInfo() {
        return __awaiter(this, void 0, void 0, function* () {
            const { code, data } = yield this._context.getRTCRoomInfo(this.roomId);
            if (code === ErrorCode$1.SUCCESS) {
                return data;
            }
            return Promise.reject(code);
    ...
    setUserInfo(info) {
        return __awaiter(this, void 0, void 0, function* () {
            const code = yield this._context.setRTCUserInfo(this.roomId, info.key, info.value);
            return code === ErrorCode$1.SUCCESS ? code : Promise.reject(code);
        });
    }
    removeUserInfo(info) {
        return __awaiter(this, void 0, void 0, function* () {
            const code = yield this._context.removeRTCUserInfo(this.roomId, info.keys);
            return code === ErrorCode$1.SUCCESS ? code : Promise.reject(code);
        });
    }
    setData(key, value, isInner, apiType, message) {
        return __awaiter(this, void 0, void 0, function* () {
            const code = yield this._context.setRTCData(this.roomId, key, value, isInner, apiType, message);
            return code === ErrorCode$1.SUCCESS ? code : Promise.reject(code);
        });
    }
    ...
    setRTCUserData(message, valueInfo, objectName) {
        return __awaiter(this, void 0, void 0, function* () {
            const code = yield this._context.setRTCTotalRes(this.roomId, message, valueInfo, objectName);
            return code === ErrorCode$1.SUCCESS ? code : Promise.reject(code);
        });
    }
    getData(keys, isInner, apiType) {
        return __awaiter(this, void 0, void 0, function* () {
            const { code, data } = yield this._context.getRTCData(this.roomId, keys, isInner, apiType);
    ...
    removeData(keys, isInner, apiType, message) {
        return __awaiter(this, void 0, void 0, function* () {
            const code = yield this._context.removeRTCData(this.roomId, keys, isInner, apiType, message);
            return code === ErrorCode$1.SUCCESS ? code : Promise.reject(code);
        });
    }
    ...
    setState(content) {
        return __awaiter(this, void 0, void 0, function* () {
            const code = yield this._context.setRTCState(this.roomId, content.report);
            return code === ErrorCode$1.SUCCESS ? code : Promise.reject(code);
        });
    }
    getUserList() {
        return __awaiter(this, void 0, void 0, function* () {
            const { code, data } = yield this._context.getRTCUserInfoList(this.roomId);
            if (code === ErrorCode$1.SUCCESS) {
                return data;
            }
            return Promise.reject(code);
    ...
    getToken() {
        return __awaiter(this, void 0, void 0, function* () {
            const { data, code } = yield this._context.getRTCToken(this.roomId, this._options.mode, this._options.broadcastType);
            if (code === ErrorCode$1.SUCCESS) {
                return data;
            }
            return Promise.reject(code);
    ...
    ping() {
        return __awaiter(this, void 0, void 0, function* () {
            const code = yield this._context.rtcPing(this.roomId, this._options.mode, this._options.broadcastType);
            return code === ErrorCode$1.SUCCESS ? code : Promise.reject(code);
        });
    }
    send(options) {
        return __awaiter(this, void 0, void 0, function* () {
            const { code, data } = yield this._context.sendMessage(ConversationType$1.RTC_ROOM, this.roomId, {
...
const mergeConversationList = (option) => {
    ...
    forEach(invalidDataIndexList, (invalidIndex) => {
        const conversation = tranReceiveConversation(newList[invalidIndex]);
...
const init = (options) => {
    ...
    assert('options.detect', options.detect, (value) => {
        return isHttpUrl(value.url) && isNumber(value.intervalTime);
    });
    const context = APIContext.init(options.appkey, options.debug, options.navigators, options.cppProtocol, options.customCMP);
    let token = '';
    ...
    imInstance = {
        ...
        rtcInnerUnwatch() {
            rtcInnerMsgWatcher.length = rtcInnerStatusWatcher.length = 0;
            context.assignWatcher({ rtcInnerWatcher: undefined });
        },
        /**
         * 建立 IM 连接
         * @param options
        ...
        connect(options) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                token = options.token;
                const res = yield context.connect(token, true);
        ...
        reconnect() {
            return __awaiter(this, void 0, void 0, function* () {
                const res = yield context.reconnect();
                if (res.code === ErrorCode$1.SUCCESS) {
                    return { id: res.userId };
                }
                return Promise.reject({ code: res.code, msg: ERROR_CODE[res.code] });
         ...
         */
        getConnectedTime() {
            return context.getConnectedTime();
        },
        /**
         * 获取 IM 连接状态
         */
        getConnectionStatus() {
            return context.getConnectionStatus();
        },
        /**
         * 获取 IM 连接用户的 id
         */
        ...
        getFileToken(fileType, fileName) {
            ...
            return context.getFileToken(fileType, fileName);
        },
        /**
         * 获取文件上传后的下载地址
         */
        ...
        */
        uploadRes) {
            ...
            return context.getFileUrl(fileType, filename, oriname, uploadRes);
        },
        /**
        ...
        Conversation: {
            ...
            getList(options) {
                return __awaiter(this, void 0, void 0, function* () {
                    const { code, data } = yield context.getConversationList(options === null || options === void 0 ? void 0 : options.count);
                    if (code === ErrorCode$1.SUCCESS && data) {
            ...
            get(options) {
                ...
                return new Conversation(context, options);
            },
            remove(options) {
                assert('options.type', options.type, isValidConversationType, true);
                return new Conversation(context, options).destory();
            },
            /**
             * 获取当前所有会话的消息未读数
             */
            getTotalUnreadCount() {
                return __awaiter(this, void 0, void 0, function* () {
                    const { code, data } = yield context.getTotalUnreadCount();
                    // 当未读数为空时，返回 0 故不校验 data 值
                    if (code === ErrorCode$1.SUCCESS) {
                        return data;
                    }
         ...
         * RTC SDK 中为构造函数
        */
        RTC: function (options) {
            assert('options.id', options.id, notEmptyString, true);
            return new RTCClient(options, context);
        },
        // RTC: {
        //   /**
        ...
        //     assert('options.roomId', options.roomId, notEmptyString, true)
        //     return new RTCClient(options, context)
        //   }
        // },
        /**
        ...
        ChatRoom: {
            ...
            get(option) {
                ...
                return new Chatroom(context, option.id);
            }
        }
    };
    return imInstance;
